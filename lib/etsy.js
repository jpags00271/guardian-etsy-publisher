...
async function getListingImages(accessToken, shopId, listingId) {
  // Note: unlike the upload endpoint (which is shop-scoped:
  // POST /shops/{shop_id}/listings/{listing_id}/images), Etsy's read endpoint for
  // a listing's images is NOT shop-scoped - it's just /listings/{listing_id}/images,
  // the same pattern as getListing(). Including shopId in this URL 404s.
  // shopId is kept as a parameter (unused) so callers don't need to change.
  const res = await fetch(`${ETSY_BASE}/listings/${listingId}/images`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'x-api-key': apiKeyHeader(),
    },
