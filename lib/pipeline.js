...
// images, order, price, tags, and listing state all stay exactly as they
// are. For the case where a listing was already published with no
// deliverable file (digitalFileId was missing at publish time) and the file
// has since been generated and recorded in manifest.json.
async function attachDigitalFile(folderId) {
  const log = [];
  const step = (msg) => {
    log.push(`[${new Date().toISOString()}] ${msg}`);
  };
  ...
  const manifestBuf = await drive.downloadFile(manifestFile.id, driveToken);
  const manifest = JSON.parse(manifestBuf.toString('utf-8'));
  step(`Manifest loaded for "${manifest.designName}"`);

  if (!manifest.listingId) {
    throw new Error('Manifest has no listingId - nothing to attach a file to yet. Run publishDesign first.');
  }
  if (!manifest.digitalFileId) {
    throw new Error('Manifest has no digitalFileId - nothing to attach. Generate the delivery PDF first.');
  }
  ...
  step(`Uploaded digital file (${pdfBuf.length} bytes) to existing listing ${manifest.listingId}`);

  manifest.digitalFileAttachedAt = new Date().toISOString();
  await updateManifest(driveToken, folderId, manifestFile.id, manifest);
  step('Manifest updated: digitalFileAttachedAt set');

  return { success: true, listingId: manifest.listingId, log };
}
...
module.exports = { publishDesign, scanQueue, attachDigitalFile };
